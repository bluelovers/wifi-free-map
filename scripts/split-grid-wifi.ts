...
function extractLocationInfo(address: string): { zipCode: string; city: string; district: string; road: string } {
    ...
    // 提取郵遞區號（3碼或5碼數字）
    const zipMatch = address.match(/^(\d{3,5})/);
    const zipCode = zipMatch ? zipMatch[1] : "";

    // 嘗試匹配縣市（XX市 或 XX縣）
    const cityMatch = address.match(/([^\d\s]+(?:市|縣))/);
    const city = cityMatch ? cityMatch[1] : "";

    // 嘗試匹配行政區（XX區 或 XX市）
    let remaining = address;
    if (city) remaining = address.replace(city, "");
    const districtMatch = remaining.match(/([^\d\s]+(?:區|市|鎮|鄉))/);
    const district = districtMatch ? districtMatch[1] : "";

    // 嘗試匹配路名（不包含門牌號碼）
    const roadMatch = address.match(/[^\d\s]+(?:路|街|大道)[一二三四五六七八九十]*(?:段)?/);
